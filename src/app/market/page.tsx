    ...
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-2 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
          <div className="rounded bg-slate-50 p-2">必填：价格、成色、可交易时间</div>
          <div className="rounded bg-slate-50 p-2">建议：实拍图 3 张以上</div>
          <div className="rounded bg-slate-50 p-2">交易方式：自提点优先</div>
          <div className="rounded bg-slate-50 p-2">完成后互评，累计信誉分</div>
        </div>
      </section>
      ...
      {marketPosts.map((post) => (
        <article key={post.id} className="rounded-xl border border-slate-200 bg-white p-4">
          <div className="flex items-center justify-between text-xs text-slate-500">
            <span>{post.author} · {post.building}</span>
